...
/** @brief Logical I2c channel assigned to the OLED (FlexIO channel, index 1). */
#define OLED_I2C_CHANNEL        I2C_MASTER_1

/** @brief 7-bit I2C address of the SSD1306 controller             */
#define OLED_ADDR               0x3C

/** @brief GPIO port half holding the OLED reset line (PTA13 -> PTA low half). */
#define OLED_RST_PORT           PTA_L_HALF

/** @brief Pin index of the OLED reset line inside OLED_RST_PORT (PTA13).
 *
 * Port_Init() configures MSCR 13 as a GPIO output with an initial level of 0,
 * which holds the SSD1306 in reset. The controller ignores all I2c traffic
 * while this line is low, so SSD1306_Init() must release it before sending the
 * initialisation command sequence.
 */
#define OLED_RST_PIN            13U
