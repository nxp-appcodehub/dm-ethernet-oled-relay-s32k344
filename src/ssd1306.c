...
/** @brief Last I2c transaction status. Kept for inspection in a debugger. */
static Std_ReturnType SSD1306_LastStatus = E_OK;
...
/**
 * @brief   Transmit a buffer to the SSD1306 over the AUTOSAR MCAL I2c API.
 * @details Uses a single blocking (synchronous) transfer with a stop
 *          condition at the end (RepeatedStart = FALSE). The transfer status
 *          is stored in SSD1306_LastStatus for optional inspection.
 * @param[in] data    Pointer to the bytes to send.
 * @param[in] length  Number of bytes to send.
 */
static void SSD1306_Transmit(const uint8_t *data, uint32_t length) {
	I2c_RequestType request = { 0 };

	request.SlaveAddress         = OLED_ADDR;
	request.BitsSlaveAddressSize = FALSE;
	request.ExpectNack           = FALSE;
	request.RepeatedStart        = FALSE;
	request.BufferSize           = (uint16) length;

	request.DataDirection        = I2C_SEND_DATA;
	request.DataBuffer           = (uint8 *) data;

	SSD1306_LastStatus = I2c_SyncTransmit(OLED_I2C_CHANNEL, &request);
}
...
static void SSD1306_WriteCmd(uint8_t cmd) {
	...
	SSD1306_Transmit(txBuffer, 2U);
}

/**
 * @brief   Send a block of data bytes to the SSD1306 GDDRAM.
...
static void SSD1306_WriteData(const uint8_t *data, uint32_t length) {
	...
	SSD1306_Transmit(txBuffer, length + 1U);
}
...
void SSD1306_Init(void) {
	/* Hardware reset sequence. Port_Init() leaves the reset line (PTA13) driven
	 * low, which holds the controller in reset and makes it ignore all I2c
	 * traffic. Pulse it high-low-high to release the controller, then allow it
	 * to settle before sending the initialisation command sequence. */
	Siul2_Dio_Ip_WritePin(OLED_RST_PORT, OLED_RST_PIN, 1);
	SSD1306_DelayMs(10U);
	Siul2_Dio_Ip_WritePin(OLED_RST_PORT, OLED_RST_PIN, 0);
	SSD1306_DelayMs(20U);
	Siul2_Dio_Ip_WritePin(OLED_RST_PORT, OLED_RST_PIN, 1);
	SSD1306_DelayMs(50U);
